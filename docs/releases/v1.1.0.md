...
- Historical note: this release originally recovered project-local `output/` data during desktop startup.
- Current policy has since removed automatic `output/` recovery; preserve old data through manual backups or explicit path configuration.
...
- Current storage policy has since been unified: Web/backend standalone and the desktop app default to `~/Downloads/Pixiv Platform/`.
